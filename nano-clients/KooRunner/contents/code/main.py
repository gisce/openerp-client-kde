...
class KooRunner(Runner):
    ...
    def init(self):
        ...
        self.addSyntax(Plasma.RunnerSyntax("%s :q:" %
    ...
    def match(self, context):
        ...
        results = Rpc.session.call(
            '/fulltextsearch', 'search', text, self.limit, 0, False, Rpc.session.context)
        for result in results:
            match = Plasma.QueryMatch(self.runner)
            match.setType(Plasma.QueryMatch.PossibleMatch)
            match.setIcon(KIcon('games-hint'))
            label = '%s: %s' % (
                result['model_label'], self.stripTags.sub('', result['headline']))
            label = label.replace('\n', '')
            match.setText(label)
    ...
    def reloadConfiguration(self):
        ...
        self.language = Settings.value(
            'runner.language', Settings.value('client.language'))
