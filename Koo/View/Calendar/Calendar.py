...
class GraphicsDayItem(QGraphicsItemGroup):
    ...
    def setSize(self, size):
        self._size = size
        self._background.setRect(
            QRectF(0, 0, self._size.width(), self._size.height()))
...
class GraphicsCalendarItem(QGraphicsItemGroup):
    ...
    def updateCalendarView(self):
        ...
        for x in range(self._startDate.daysTo(self._endDate) + 1):
            ...
            item.setDate(date)
            item.setPos((x + offset) % 7 * dayWidth,
                        dayHeight * ((x + offset) // 7))
...
(CalendarViewUi, CalendarViewBase) = loadUiType(
    Common.uiPath('calendarview.ui'))
