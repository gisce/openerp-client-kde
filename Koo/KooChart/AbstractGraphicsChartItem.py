...
class ColorManager:
    colorList = [QColor('#%02x%02x%02x' % (25 + ((r + 10) % 11) * 23, 5 + ((g + 1) % 11) *
                                           20, 25 + ((b + 4) % 11) * 23)) for r in range(11) for g in range(11) for b in range(11)]
...
class LegendItem(QGraphicsItemGroup):
    ...
    def updateLabels(self):
        ...
        maxHeight = max(text.boundingRect().bottom(),
    ...
    def place(self):
        ...
        for x in range(len(items) + 1):
            if x > 0:
                i = items[x - 1].mapToScene(items[x -
                                                  1].boundingRect()).boundingRect().right()
            else:
                i = 0
            if x < len(items):
                j = items[x].mapToScene(
                    items[x].boundingRect()).boundingRect().x()
            else:
                j = self.chartWidth()
            width = j - i
            if maxWidth < width:
                maxWidth = width
                maxWidthPosition = i
        self.setPos(maxWidthPosition + maxWidth / 2 -
                    self.boundingRect().width() / 2, self.pos().y())
