...
def exportHtml(fname, fields, result, writeTitle):
    ...
    try:
        f.write('<html>')
        f.write('<head><title>' +
                _('OpenERP exported information') + '</title></head>')
        f.write('<table>')
        if writeTitle:
            f.write('<tr>')
            for x in fields:
...
def openExcel(fields, fieldsType, result, writeTitle):
    ...
    try:
        ...
        sheet.Range(sheet.Cells(row, 1), sheet.Cells(
            row + len(result) - 1, len(fields))).Value = result
    ...
    try:
        ...
        for i in range(30):
            ...
            if ooo and ooo.desktop:
                break
            time.sleep(1)
        doc = ooo.desktop.loadComponentFromURL(
...
def exportData(ids, model, fields, importCompatible):
    ...
    data = Rpc.session.execute(
        '/object', 'execute', model, 'export_data', ids, fields, context)
...
class ExportDialog(QDialog, ExportDialogUi):
    ...
    def export(self):
        ...
        if 'warning' in result:
            QMessageBox.warning(
                self, _('Data Export Error'), result['warning'])
            return
        result = result['data']
    ...
    def save(self):
        name, ok = QInputDialog.getText(
            self, '', _('What is the name of this export?'))
        if not ok:
            return
        ir_export = Rpc.RpcProxy('ir.exports')
        fields = []
...
if OpenOffice.isOpenOfficeAvailable:
    ExportDialog.registerExport('openoffice', _(
        'Open with OpenOffice.org'), False, openOpenOffice)
# Add Excel export only if it's available
if os.name == 'nt':
    try:
        from win32com.client import Dispatch
        application = Dispatch("Excel.Application")
        ExportDialog.registerExport('excel', _(
            'Open with Excel'), False, openExcel)
    except:
        pass
...
class StoredExportsModel(QStandardItemModel):
    ...
    def load(self, model, fieldsInfo):
        ...
        for export in ir_export.read(export_ids):
            ...
            items = [QStandardItem(', '.join([f['name'] for f in fields])), QStandardItem(str(export['id'])), QStandardItem(
                export['name']), QStandardItem(', '.join([fieldsInfo[f['name']]['string'] for f in fields]))]
