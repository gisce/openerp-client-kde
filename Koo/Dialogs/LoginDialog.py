...
class LoginDialog(QDialog, LoginDialogUi):
    ...
    def refreshList(self):
        res = ServerConfigurationDialog.refreshDatabaseList(
            self.uiDatabase, str(self.uiServer.text()), self.databaseName)
        if res == -1:
            self.uiNoConnection.setText(
        ...
        try:
            Common.serverVersion = Rpc.database.call(
                str(self.uiServer.text()), 'server_version')
            Common.serverMajorVersion = '.'.join(
                Common.serverVersion.split('.')[0])
