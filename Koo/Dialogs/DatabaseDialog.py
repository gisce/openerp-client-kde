...
class DatabaseDialog(QDialog, DatabaseDialogUi):
    ...
    def refreshList(self):
        res = ServerConfigurationDialog.refreshDatabaseList(
            self.uiDatabaseSelector, str(self.uiServer.text()))
        if res == -1:
            self.uiInformation.setText(
            ...
            self.uiInformation.setText(
                '<b>' + _('No database found, you must create one !') + '</b>')
            self.uiInformation.setVisible(True)
            self.uiDatabaseSelector.setEnabled(False)
            self.uiDatabaseEditor.setEnabled(False)
            self.pushAccept.setEnabled(False)
...
def restoreDatabase(parent):
    ...
    try:
        ...
        return
    parent.unsetCursor()
    QMessageBox.information(parent, _('Information'),
...
def backupDatabase(parent):
    ...
    parent.setCursor(Qt.WaitCursor)
    try:
        dump_b64 = Rpc.database.execute(
            dialog.url, 'dump', dialog.password, dialog.databaseName)
    ...
    parent.unsetCursor()
    QMessageBox.information(parent, _('Information'),
                            _("Database backuped successfully!"))
...
def dropDatabase(parent):
    ...
    try:
        ...
        if e.message == 'AccessDenied:None':
            ...
            QMessageBox.warning(parent, _("Database removal"),
                                _("Couldn't drop database"))
        return
    parent.unsetCursor()
    QMessageBox.information(parent, _('Database removal'),
                            _('Database dropped successfully!'))
