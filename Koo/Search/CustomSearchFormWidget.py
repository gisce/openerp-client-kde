...
(CustomSearchItemWidgetUi, CustomSearchItemWidgetBase) = loadUiType(
    Common.uiPath('customsearchitem.ui'))
...
class CustomSearchItemWidget(AbstractSearchWidget, CustomSearchItemWidgetUi):
    ...
    operators = (
        ...
        ('is not empty', _('is not empty'), ('char', 'text',
                                             'many2one', 'date', 'time', 'datetime', 'float_time'), False),
        ('ilike', _('contains'), ('char', 'text',
                                  'many2one', 'many2many', 'one2many'), True),
        ('not ilike', _('does not contain'), ('char', 'text', 'many2one'), True),
        ('=', _('is equal to'), ('boolean', 'char', 'text', 'integer',
                                 'float', 'date', 'time', 'datetime', 'float_time', 'user'), True),
        ('<>', _('is not equal to'), ('boolean', 'char', 'text', 'integer',
                                      'float', 'date', 'time', 'datetime', 'float_time'), True),
        ('>', _('greater than'), ('char', 'text', 'integer',
                                  'float', 'date', 'time', 'datetime', 'float_time'), True),
        ('<', _('less than'), ('char', 'text', 'integer', 'float',
                               'date', 'time', 'datetime', 'float_time'), True),
        ('>=', _('greater or equal to'), ('char', 'text', 'integer',
                                          'float', 'date', 'time', 'datetime', 'float_time'), True),
        ('<=', _('less or equal to'), ('char', 'text', 'integer',
                                       'float', 'date', 'time', 'datetime', 'float_time'), True),
        ('in', _('in'), ('selection', 'char', 'text', 'integer',
                         'float', 'date', 'time', 'datetime', 'float_time'), True),
        ('not in', _('not in'), ('selection', 'char', 'text', 'integer',
                                 'float', 'date', 'time', 'datetime', 'float_time'), True),
        ...
        if fieldType in self.typeRelated:
            ...
            if model:
                view = Rpc.session.execute(
                    '/object', 'execute', model, 'fields_view_get', False, 'form', Rpc.session.context)
    ...
    def value(self):
        ...
        if operator in ('in', 'not in'):
            ...
            for item in value.split(','):
                data = self.correctValue(
                    item.strip(), fieldName, relatedFieldName)
        ...
        if fieldType == 'user':
            data = Rpc.session.execute(
                '/object', 'execute', 'res.users', 'name_search', value)
            if data:
                value = data[0][0]
                text = data[0][1]
            else:
