...
def data_files():
    ...
    files = [
        ...
        (opj('share', 'doc', 'koo', 'manual'), [f for f in glob.glob(
            opj('doc', 'html', '*')) if os.path.isfile(f)]),
        (opj('share', 'doc', 'koo', 'api'), [f for f in glob.glob(
            opj('doc', 'doxygen', 'html', '*')) if os.path.isfile(f)]),
    ...
    if using_py2exe:
        ...
        files.append(
            (opj('share', 'Koo', 'Plugins', 'RemoteHelp', 'data'),
             glob.glob(opj('Koo', 'Plugins', 'RemoteHelp', 'data', '*'))),
        )
...
def findPluginsDir(module):
    result = []
    plugins = [x for x in glob.glob(
        opj('Koo', module, '*')) if os.path.isdir(x)]
